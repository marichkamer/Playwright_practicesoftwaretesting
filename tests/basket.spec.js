...
test.describe('Basket Functionality', () => {
    ...
    test('001-[BASKET-01] User can add a product to the basket @public', async ({ page }) => {
        const homePage = new HomePage(page);
        await homePage.addProductToCart(productName.hammer);
        await expect(homePage.cartQuantity).toHaveText('1');
    });

    test ('002-[BASKET-02] User can remove a product from the basket @public', async ({ page }) => {
        const homePage = new HomePage(page);
